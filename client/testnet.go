...
var (
	...
	PowerReduction       = math.NewIntFromBigInt(new(big.Int).Exp(big.NewInt(10), big.NewInt(BaseDenomUnit), nil))
)
...
Example:
	...
	cmd.Flags().String(flagNodeDaemonHome, "scalerized", "Home directory of the node's daemon configuration")
	cmd.Flags().String(flagStartingIPAddress,
		"192.168.0.1",
		"Starting IP address (192.168.0.1 results in persistent peers list ID0@192.168.0.1:46656, ID1@192.168.0.2:46656, ...)")
	cmd.Flags().String(flagIPAddresses, "", "List of IP addresses of the testnodes")
...
Example:
	...
	cmd.Flags().String(flagGRPCAddress, srvconfig.DefaultGRPCAddress, "the gRPC server address to listen on")
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("scalerize_%d-1", cmtrand.Int63n(9999999999999)+1)
	}
	...
	if args.ipAddresses != "" {
		ipAddresses = strings.Split(args.ipAddresses, ",")

		if len(ipAddresses) != args.numValidators {
			return fmt.Errorf("invalid number of IP addresses")
		}
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(params.CoinUnit, accStakingTokens),
		}
		...
		valTokens := sdk.TokensFromConsensusPower(100, PowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			valStr,
			valPubKeys[i],
			sdk.NewCoin(params.CoinUnit, valTokens),
		...
		// customAppTemplate, customAppConfig := config.DefaultConfig()
		srvconfig.SetConfigTemplate(srvconfig.DefaultConfigTemplate)

		srvconfig.WriteConfigFile(filepath.Join(nodeDir, "config/app.toml"), appConfig)
	}

	if err := initGenFiles(clientCtx, mbm, args.chainID, params.CoinUnit, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
